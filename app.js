...
async function loadOffers(){const store=currentStore();if(!state.items.length||!store.retailer){state.offers={};save();render();return}$("#syncState").textContent="Angebote werden aktualisiert …";try{const res=await fetch("/api/match-offers",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({retailer:store.retailer,items:state.items.map(x=>({id:x.id,name:x.name}))})});if(!res.ok)throw new Error();const data=await res.json();state.offers=data.matches||{};save();$("#syncState").textContent="Angebote online aktualisiert";render()}catch(e){$("#syncState").textContent="Angebote gerade nicht erreichbar · Liste funktioniert offline weiter";}}
